            ...
            if ((path == NULL && tree_view->priv->hover_path != NULL) || (path != NULL && tree_view->priv->hover_path == NULL) || (path != NULL && tree_view->priv->hover_path != NULL && gtk_tree_path_compare(path, tree_view->priv->hover_path) != 0))
            {
                ...
                if (G_LIKELY(tree_view->priv->single_click_timeout > 0 && tree_view->priv->hover_path != NULL))
                {
                    ...
                    tree_view->priv->single_click_timeout_id = gdk_threads_add_timeout_full(G_PRIORITY_LOW, tree_view->priv->single_click_timeout,
                                                                                            exo_treeview_single_click_timeout, tree_view,
                                                                                            exo_treeview_single_click_timeout_destroy);
                }
            }
            else
            {
...
/**
 * exo_treeview_get_single_click_timeout:
