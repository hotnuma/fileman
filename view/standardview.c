...
static void standardview_view_init(BaseViewIface *iface);
...
static void standardview_set_loading(StandardView *view,
                                     gboolean loading);
...
static void _standardview_scrolled(GtkAdjustment *adjustment,
                                   StandardView *view);
...
static void _standardview_current_directory_destroy(
                                        ThunarFile *current_directory,
                                        StandardView *view);
static void _standardview_current_directory_changed(
                                        ThunarFile *current_directory,
                                        StandardView *view);
...
G_DEFINE_ABSTRACT_TYPE_WITH_CODE(StandardView, standardview,
...
static void standardview_set_zoom_level(BaseView *baseview, ThunarZoomLevel zoom_level)
{
    ...
    if (G_LIKELY(view->priv->zoom_level != zoom_level))
    {
...
static void _standardview_row_changed(ListModel    *model,
                                      GtkTreePath  *path,
                                      GtkTreeIter  *iter,
                                      StandardView *view)
{
...
static void _standardview_rows_reordered(ListModel    *model,
                                         GtkTreePath  *path,
                                         GtkTreeIter  *iter,
                                         gpointer     new_order,
                                         StandardView *view)
                                                    ...
                                                    StandardView *view)
{
    ...
    window_update_directories(THUNAR_WINDOW(window), current_directory,
                              new_directory);
                                        ...
                                        StandardView     *view)
{
    ...
    {
        gchar **uris = e_file_list_to_stringv(view->priv->drag_g_file_list);
    ...
    if (G_LIKELY(!view->priv->drop_data_ready))
    {
        ...
        if (info == TARGET_TEXT_URI_LIST
            ...
            view->priv->drop_file_list =
                    e_file_list_new_from_string((gchar*) gtk_selection_data_get_data(
                                                    selection_data));
        ...
        else if (G_LIKELY(info == TARGET_TEXT_URI_LIST))
        {
            ...
            if (G_LIKELY((actions & (GDK_ACTION_COPY | GDK_ACTION_MOVE | GDK_ACTION_LINK)) != 0))
            {
                ...
                if (G_LIKELY(action != 0))
                {
                    ...
                    succeed = dnd_perform(GTK_WIDGET(view), file, view->priv->drop_file_list,
                                                  action, _standardview_new_files_closure(view, source_view));
                }
            }
