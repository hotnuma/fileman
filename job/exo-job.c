...
static guint _exo_job_signals[LAST_SIGNAL];
...
static gboolean _exo_job_async_ready(gpointer user_data)
{
    ...
    if (job->priv->failed)
    {
        ...
        if (!g_error_matches(job->priv->error,
...
gboolean exo_job_set_error_if_cancelled(ExoJob *job, GError **error)
{
    ...
    return g_cancellable_set_error_if_cancelled(job->priv->cancellable,
                                                error);
}
...
void exo_job_info_message(ExoJob *job, const gchar *format, ...)
{
    ...
    exo_job_emit(job, _exo_job_signals[INFO_MESSAGE], 0, message);
